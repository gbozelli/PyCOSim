...
def photodiode(s=None,t=None,R=0.9,BW=10e9,shot_noise=True,thermal_noise=True):
    ...
    if shot_noise:
        ...
        n_shot = np.sqrt(P_n_shot)*np.random.randn(N)*1e6
    else:
        n_shot = 0
...
def EDFA(s_in,t,G_edfa_dB,NF_dB,lambda0=1550e-9):
    ...
    F_n = 10**(NF_dB/10)
    n_sp = F_n/2
    S_ASE = n_sp*h*f0*max(0, G_edfa-1)
    P_ASE = S_ASE*BW_sim
...
def QAM_receiver_DP(s,t,s_b,M,N_sync=128,N_MIMO=32,N_inf=4096,SpS=16,RollOff=0.2,sync_seed_X=0,sync_seed_Y=123,ts=1e-12,plot_flag=False,L=10e3,D=16e-6):
    ...
    _rec("constellation_x", sx_inf_pc)
    _rec("constellation_y", sy_inf_pc)
...
def sweep(sc: Scenario, what: str):
    if what == "power":
        values = np.arange(-4, 10, 1)  # dBm values
        sc.L_span = 120e3
    else:
        values = np.arange(50e3, 210e3, 25e3)
    out = []
    ber_x_list = []
    ber_y_list = []
    plot_values = []
    ...
    for v in values:
        if what == "power": 
            sc.P_LOP = float((10**(v/10))/1000)
            print(f"\n--- {what} = {v:.4g} dBm ---")
        else:               
            sc.L_span, sc.N_span = float(v), 1
            print(f"\n--- {what} = {v/1e3:.4g} km ---")
            
        r = run(sc, "span")
        out.append({what: float(v), "ber": r["ber"]})
        
        if "constellation_x" in RECORD and "constellation_y" in RECORD and plt is not None:
            cx = RECORD["constellation_x"]
            cy = RECORD["constellation_y"]
            plt.figure(figsize=(10, 5))
            plt.subplot(1, 2, 1)
            plt.scatter(np.real(cx[-8000:]), np.imag(cx[-8000:]), s=1, alpha=0.5, c='b')
            unit = f"{v} dBm" if what == "power" else f"{v/1e3} km"
            plt.title(f"Pol X ({unit})")
            plt.axis('square')
            plt.grid(True, ls="--")
            plt.subplot(1, 2, 2)
            plt.scatter(np.real(cy[-8000:]), np.imag(cy[-8000:]), s=1, alpha=0.5, c='r')
            plt.title(f"Pol Y ({unit})")
            plt.axis('square')
            plt.grid(True, ls="--")
            plt.tight_layout()
            plt.savefig(f"{out_dir}/constellation_{what}_{v}.png", dpi=150)
            plt.close()
            
        b = r["ber"]
        if len(b) > 0 and len(b[0]) > 0:
            ber_x_list.append(b[0][0][0])
            ber_y_list.append(b[0][0][1])
            plot_values.append(v)
            
    if plt is not None and len(plot_values) > 0:
        plt.figure(figsize=(8, 6))
        plt.semilogy(plot_values, ber_x_list, 'o-', label='Pol X')
        plt.semilogy(plot_values, ber_y_list, 's-', label='Pol Y')
        plt.xlabel('Launch Power (dBm)' if what == "power" else 'Length (km)')
        plt.ylabel('BER')
        plt.title(f'BER vs {what}')
        plt.grid(True, which="both", ls="--")
        plt.legend()
        plt.savefig(f"{out_dir}/ber_curve_{what}.png", dpi=150)
        plt.close()
